...
echo.
echo [1/4] Compiling GIDR v6.3.0...
"%CSC%" /nologo /target:exe /out:bin\GIDR.exe /platform:anycpu /optimize+ ^
...
echo [+] Compile succeeded: bin\GIDR.exe
...
if /i "%MODE%"=="sign" (
    ...
    "!SIGNTOOL!" sign /f "%PFX%" /p "%PASS%" /tr http://timestamp.sectigo.com /td sha256 /fd sha256 /v bin\GIDR.exe
    if %ERRORLEVEL% NEQ 0 (
        echo [!] Signing FAILED.
        exit /b 1
    )
    ...
    "!SIGNTOOL!" verify /pa bin\GIDR.exe >nul 2>&1
    if %ERRORLEVEL% EQU 0 (
...
echo.
echo ============================================
echo   GIDR v6.3.0 - Build complete!
echo.
if exist bin\GIDR.exe echo   EXE:       bin\GIDR.exe
echo.
echo   Usage:
