...
:: Gorstak EDR - Code Signing Script
...
echo [*] Signing GEdr.exe...
"%SIGNTOOL%" sign /f "%PFX%" /p "%PASS%" /tr %TIMESTAMP% /td sha256 /fd sha256 /v bin\GEdr.exe
if %ERRORLEVEL% NEQ 0 (
    echo [!] Signing FAILED for GEdr.exe
    exit /b 1
)
...
"%SIGNTOOL%" verify /pa /v bin\GEdr.exe
echo.
