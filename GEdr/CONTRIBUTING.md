...
4. Run `bin\GEdr.exe info` to verify the build works

No Visual Studio or SDK is required. Any text editor works.

## Project Architecture

GEdr is a single-executable EDR built with raw `csc.exe` compilation. All `.cs` files in the project tree are compiled together via `/recurse:*.cs`.
...
System-critical processes are listed in `Config.cs` under `ProtectedProcesses`. The EDR will refuse to terminate these. If your detection might flag a legitimate Windows process, add it to this list.

## Testing

Run the EDR in monitor mode on a clean system and verify:
...
```
bin\GEdr.exe scan C:\Windows\System32 -r --no-action
```
...
- Whether YARA was enabled (`GEdr.exe info`)
