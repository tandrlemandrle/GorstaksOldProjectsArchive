...
{
    /// <summary>
    /// Monitors new process creation via WMI events. When a new process starts,
    /// feeds its executable through the scan pipeline. Falls back to polling if
    /// WMI events are unavailable.
    /// Ported from GorstaksEDR Start-ProcessMonitor (WMI -> CIM -> polling fallback).
    /// </summary>
    public static class ProcessMonitor
    {
        ...
        private static void AnalyzeNewProcess(int pid, string name, string exePath, string cmdLine)
        {
            ...
            // Scan the executable if it exists
            if (!string.IsNullOrEmpty(exePath) && File.Exists(exePath) && !Config.IsExcludedPath(exePath))
            {
                try
                {
                    ScanResult result = ScanPipeline.ScanFile(exePath);

                    if (result.TotalScore >= Config.RuntimeAlertThreshold)
                    {
                        Logger.Log(string.Format("ProcessMonitor: {0} (PID:{1}) score:{2} verdict:{3}",
                            name, pid, result.TotalScore, result.Verdict), LogLevel.THREAT, "process_monitor.log");

                        ThreatInfo threat = new ThreatInfo();
                        threat.ThreatType = "NewProcess:" + result.Verdict;
                        ...
                        {
                            threat.YaraRule = result.YaraMatches[0].RuleName;
                            for (int i = 0; i < result.YaraMatches.Count; i++)
                                threat.DetectionMethods.Add("YARA:" + result.YaraMatches[i].RuleName);
                        }

                        ResponseQueue.Enqueue(threat);

                        if (Config.AutoKillThreats && result.Severity >= ThreatSeverity.Critical)
                            ThreatActions.TerminateProcess(pid, name);

                        if (Config.AutoQuarantine && result.Severity >= ThreatSeverity.High)
                            ThreatActions.Quarantine(exePath, "ProcessMonitor: " + result.Verdict);
                    }
                }
                catch (Exception ex)
                {
                    Logger.Log(string.Format("ProcessMonitor scan error for {0}: {1}", name, ex.Message), LogLevel.WARN);
                }
            }
